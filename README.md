...
| 테스트 | **571개** (서비스 · API · 동시성 · 실시간 · 암호화 대조) |
...
- **정지·탈퇴는 이미 발급된 토큰에도 곧바로 적용됩니다.** JWT 는 발급 시점 값이라 그대로 두면 정지한 사람이 토큰 만료(30분)까지 채팅을 더 보낼 수 있었습니다. 인증 필터가 회원의 현재 상태·권한을 보되, 매 요청 DB 조회를 피하려고 **회원당 60초 메모리 캐시**를 두고 정지·해제·탈퇴 때 **커밋 전후로 두 번** 비웁니다. 조회하는 도중 캐시가 비워졌으면 읽은 값을 담지 않아, 정지 직전에 읽은 "정상"이 60초 동안 남는 경쟁도 막았습니다.
...
```bash
./gradlew test    # 571개, H2 라 DB 없이 돈다
...
- **로그아웃** 뒤에도 access token 은 만료(최대 30분)까지 유효합니다(정지·탈퇴는 곧바로 막습니다). 로그아웃까지 막으려면 토큰마다 블랙리스트가 필요합니다. 정지 상태 캐시도 메모리라 서버를 늘리면 비우는 신호를 서버끼리 나눠야 합니다.
