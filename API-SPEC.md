...
#### 정지·탈퇴 계정의 토큰 — `403 USER_NOT_ACTIVE`

토큰이 맞아도 **정지·탈퇴한 회원이면 모든 요청(공개 API 포함)이 `403 USER_NOT_ACTIVE`** 다. 토큰은 발급 시점 값이라, 서버가 요청마다 회원의 현재 상태를 본다(회원당 60초 메모리 캐시, 정지·해제·탈퇴하면 그 회원의 캐시를 곧바로 비운다). 권한(ADMIN/USER)도 토큰이 아니라 현재 DB 값을 쓴다. 채팅 WebSocket 연결(CONNECT)도 같은 코드로 거절한다(이미 열린 연결은 끊지 않는다).

| code | 상황 | 프론트 동작 |
|---|---|---|
| `USER_NOT_ACTIVE` | 정지·탈퇴한 계정의 토큰 · 로그인 · 재발급 | 저장된 토큰 삭제 후 "이용할 수 없는 계정" 알림 |
...
- ~~이미 발급된 access token 은 만료(최대 30분)까지 통과한다.~~ (2026-09-23 변경) 다른 기기에 남은 access token 도 탈퇴 즉시 모든 요청이 `403 USER_NOT_ACTIVE` 다(공통 규약 참고). 열려 있던 WebSocket 연결은 끊지 않는다.
...
| 값 | `true`인 조건 |
|---|---|
| `reserve` | 판매자 · 이 방에 거래 없음 · 상품이 판매중(다른 방에 예약이 있으면 상품이 예약중이라 `false`) · 두 사람 사이에 차단 관계 없음(어느 쪽이 차단했든 `false`, 14절) |
...
로그인·토큰 재발급과 **이미 발급된 access token 으로 하는 모든 요청이 곧바로** `403 USER_NOT_ACTIVE`로 막힌다(2026-09-23, 공통 규약 참고). 예전에는 토큰 만료(최대 30분)까지 열려 있어, 정지한 사람이 그동안 채팅을 더 보낼 수 있었다. 요청마다 DB 를 보지 않도록 회원 상태를 60초 기억하고, 정지·해제하면 그 회원의 기억을 곧바로 비운다.
