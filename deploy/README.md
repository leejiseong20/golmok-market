...
첫 업로드(2026-09-17, 커밋 `c43a96d`)는 테스트·이미지 단계 모두 성공했고,
**이미지가 로그인 없이 받아지는 것을 확인했다**(저장소가 공개라 패키지도 공개로 생성됨). 따로 할 일은 없다.

만약 EC2에서 `docker compose pull`이 `unauthorized`로 실패하면 패키지가 비공개인 것이다.
